...
//		});
	...
	*readToBuffer { |server,path,startFrame = 0,numFrames, action, bufnum, lameopts="" |
		...
		*decodeToTmpDir { |path, action,lameopts="" |
		var tmpPath = "/tmp/sc3mp3read-" ++ UniqueID.next ++ ".wav" ;
		if(
		//(MP3.lamepath + "--decode" + lameopts + "\"" ++ path ++ "\"" + tmpPath).systemCmd == 0, 
		("afconvert" + "--file WAVE" + "--data LEI16" + "\"" ++ path ++ "\"" + tmpPath).systemCmd == 0,
		{
			^tmpPath;
		}, {
			("MP3: unable to read file:" + path).warn;

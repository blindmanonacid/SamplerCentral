...
RedCombiSampler {
	...
	*new{|server, thresh|
	...
	prepareForPlay{|key, path, startFrame= 0, numFrames|
		var f;
		if(path.extension=="mp3", {path= MP3.decodeToTmpDir(path); mp3[key]= path.deepCopy;});
		f= SoundFile.new;
		f.openRead(path);
	...
	freeKey { arg key;
		...
		keys.removeAt(key);
		if (mp3[key].isNil.not, { ("rm" + mp3[key]).unixCmd; mp3.removeAt(key) });
	}
	free {
		ramSampler.free;
		diskSampler.free;
		keys= Dictionary.new;
		mp3.do({|it| ("rm" + it).unixCmd; });
		//maybe ("rm" + "/tmp/sc3mp3read-*.*").unixCmd; to be sure...
	}
	keys {

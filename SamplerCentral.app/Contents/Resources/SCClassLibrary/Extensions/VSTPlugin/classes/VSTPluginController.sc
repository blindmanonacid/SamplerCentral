VSTPluginController {
	...
	*new { arg synth, id, synthDef, wait= -1;
		...
		id.notNil.if {
			...
			desc = plugins[id];
			desc ?? {
				MethodError("SynthDef '%' doesn't contain a VSTPlugin with ID '%'!".format(synth.defName.asSymbol, id), this).throw;
			};
		} {
			// otherwise just get the first (and only) plugin
			(plugins.size > 1).if {
				MethodError("SynthDef '%' contains more than 1 VSTPlugin - please use the 'id' argument!".format(synth.defName.asSymbol), this).throw;
			};
	...
	*prFindPlugins { arg synth, synthDef;
		...
		synthDef.notNil.if {
			...
			desc.isNil.if { MethodError("couldn't find SynthDef '%' in global SynthDescLib!".format(synth.defName.asSymbol), this).throw };
			metadata = desc.metadata; // take metadata from SynthDesc, not SynthDef (SC bug)!
		};
		plugins = metadata[\vstplugins];
		(plugins.size == 0).if { MethodError("SynthDef '%' doesn't contain a VSTPlugin!".format(synth.defName.asSymbol), this).throw; };
		^plugins;
	}

...
\csgenericrgb\c37500\c37500\c37500;\csgenericrgb\c20000\c20000\c75000;\csgenericrgb\c75000\c0\c0;\csgenericrgb\c40000\c40000\c75000;\csgenericrgb\c0\c45000\c0;
\csgenericrgb\c100000\c40000\c0;}
\pard\tx560\tx1120\tx1680\tx2240\tx2800\tx3360\tx3920\tx4480\tx5040\tx5600\tx6160\tx6720\pardirnatural

\f0\fs18 \cf2 UC33LoopChan\cf3 \{ \
\
	\cf4 var\cf3  player= \cf5 "gpLooper"\cf3 , c, buffer, <>title, syn, synOn, loadFile, loadDialog, x= 20, y= 20, s, <>knob1, knobDefaultAction, bgColor;\
	\cf4 var\cf3  view, x, y, <>out= 0, preferences;\
	\cf4 var\cf3  amp, ampspec, firstRun= \cf6 true\cf3 ;\
	\cf4 var\cf3  ctrl, shift, alt;\
	\cf4 var\cf3  <>pan= 0; \cf7 //was 1\cf3 \
	\cf4 var\cf3  panarr, outputs, <>defaultout, outbut, panbut;\
	\cf4 var\cf3  timer, timeFunc, clearTmeFunc;\
	\cf4 var\cf3  currentPath, speechout, <>output;\
	\cf4 var\cf3  panKnob;\
	\cf4 var\cf3  frames= 0, samplerate= 48000, dur= 1, synthID;\
	\cf4 var\cf3  progressFunc, progressView;\
	\cf4 var\cf3  ruleFolderNames, rulesList, rulesApply, rulesOnOff;\
	\cf4 var\cf3  errorFileMessage;\
	\cf4 var\cf3  rate= 1;\
	\cf4 var\cf3  sampler, properties, knobNum;\
	\cf4 var\cf3  builtin= \cf6 false\cf3 ;\
	\cf4 var\cf3  >allSliders;\
\
	*new \{\cf4 arg\cf3  view, x, y, out, bgColor, preferences, properties, sampler, knobNum;\
		^\cf8 super\cf3 .new.initUC33LoopChan(view, x, y, out, bgColor, preferences, properties, sampler, knobNum);\
	\}\
	\
	initUC33LoopChan \{\cf4 arg\cf3  argview, argx, argy, argout, argbgColor, argpreferences, argproperties, argsampler, argknobNum;\
\
		view= argview;\
		x= argx;\
		y= argy;\
		...
		defaultout= argout;\
		bgColor= argbgColor;\
		bgColor ?? \{bgColor= \cf2 Color\cf3 .black.alpha_(0.5)\};\
		...
		\
		timer= \cf2 Date\cf3 .getDate.bootSeconds.round(1);\
		\
...
\
		loadFile= \{\cf4 |paths|\cf3 \
			\cf4 var\cf3  maxnewsamples, numknobs;\
			case\
			\{paths.class == \cf2 Array\cf3  \} \{\
			if(paths[0].isSoundFileOrMp3 , \{\
				1.do(\{\
					...
					s.sync;\
					if(p.extension==\cf5 "mp3"\cf3 , \{p= \cf2 MP3\cf3 .decodeToTmpDir(p); \});\
					if (buffer.isNil.not, \{\
					...
					amp= 0.5; \cf7 //set sensible amplitude as default\cf3 \
...
\
					f.close;\
					\cf7 //if(progressView.isClosed.not, \{\cf3 \
						\cf7 //"pr view no tttt closef".postln;\cf3 \
					progressView.drawFunc = \{\
					\cf2 Pen\cf3 .perform(\cf9 \\fill\cf3 );\
					\cf2 Pen\cf3 .color = \cf2 Color\cf3 .white(0.9, 1);\
					\cf2 Pen\cf3 .translate(-5, 0);\
			...
			if(allSliders.isNil.not, \{\
				if(paths.size > 1, \{\
				numknobs= allSliders.size;\
				maxnewsamples= numknobs-(knobNum-1);\
				maxnewsamples.postln;\
				paths= paths.keep(maxnewsamples);\
				paths.do(\{\cf4 |it, i|\cf3 \
					if (i==0, \{\cf6 nil\cf3 \}, \{\
					allSliders[(knobNum-1)+i].setValues([it]);\
					\});\
				\});\
				\});\
			\});\
			...
			\});\
			\}\
			\{paths.class == \cf2 Integer\cf3  \} \{ \cf8 this\cf3 .setAction(\{\cf4 |v|\cf3 \{ \
				sampler.set((\cf5 "snd"\cf3 ++((paths).asString)).asSymbol).pan_(v.value.linlin(0.0, 1.0, -1.0, 1.0));\
			...
			\{paths.class == \cf2 UC33LoopChan\cf3  \} \{\
				paths.free;\
				paths.setAction(\{\cf4 |v|\cf3 \{ panKnob.valueAction_(v.value)\}.defer \}, knobNum);\
				\cf7 //buffer= paths;\cf3 \
			\}\
			\{paths.class == \cf2 SliderLooper\cf3  \} \{\
				paths.free;\
				paths.setAction(\{\cf4 |v|\cf3 \{ panKnob.valueAction_(v.value)\}.defer \}, knobNum);\
				\cf7 //buffer= paths;\cf3 \
			\};\
...
\
		...
		\};\
\
		c= \cf2 CompositeView\cf3 (view, \cf2 Rect\cf3 (x, y, 90, 115))\
		\cf7 //.relativeOrigin_(true)\cf3 \
		.background_(bgColor);\
\
...
\
		...
		progressView.drawFunc = \{\
			\cf7 //Pen.translate(100, 100);\cf3 \
				...
				\cf2 Pen\cf3 .color = \cf2 Color\cf3 .cyan(0.5, 1);\
				\cf2 Pen\cf3 .addWedge(20@20, 15, 0.75*2pi, elapsed*2pi);\
				\cf2 Pen\cf3 .perform(\cf9 \\fill\cf3 );\
				\cf2 Pen\cf3 .color = \cf2 Color\cf3 .white(1, 1);\
				\cf2 Pen\cf3 .translate(-5, 0);\
				\cf2 Pen\cf3 .stringAtPoint((remaining.floor).asTimeString.drop(3).drop(-4), 10@10);\
...
\
		\
		progressFunc.value(c, \cf2 Rect\cf3 (25, 20, 50, 50));\
	\
		knob1= \cf2 Knob\cf3 .new(c, \cf2 Rect\cf3 (10, 5, 70, 70))\
		.addAction( \{\cf4 |v,x,y,mod|\cf3 \
			if (mod.isShift, \{ loadDialog.value \});\
			if (mod.isAlt, \{ \cf8 this\cf3 .free2; \cf5 "remove file"\cf3 .postln; \{knob1.value= 0\}.defer; \});\
			if (mod.isCtrl, \{ \
				...
				\{knob1.value= 0\}.defer;\
				\});\
			\}, \cf9 \\mouseDownAction\cf3 )\
		.addAction( \{\cf4 |v,x,y,mod|\cf3 \
			\cf7 //if (mod.isShift, \{ loadDialog.value \});\cf3 \
			if (mod.isAlt, \{ \cf5 "do"\cf3 .postln; knob1.value= 0 \});\
			if (mod.isCtrl, \{ \cf5 "do"\cf3 .postln; knob1.value= 0 \});\
			\}, \cf9 \\mouseUpAction\cf3 )\
		.canFocus_(\cf6 false\cf3 )\
		.canReceiveDragHandler_(\cf6 true\cf3 )\
		.receiveDragHandler_(\{\cf4 |view|\cf3 \
			\cf4 var\cf3  drag;\
			drag= \cf2 View\cf3 .currentDrag;\
			if(drag.isString, \{drag= [drag];\}); \cf7 // qt compatibility..\cf3 \
			loadFile.value(drag);\
		\})\
		.color_([\cf2 Color\cf3 .blue(0.7, 0.5), \cf2 Color\cf3 .green(0.8, 0.8), \cf2 Color\cf3 .black.alpha_(0.3), \cf2 Color\cf3 .black.alpha_(0.7)])\
		.beginDragAction_(\{ \cf8 this\cf3 .postln; \})\
		.action_(knobDefaultAction);\
		\
\
\cf7 //"got here".postln;\cf3 \
\
		title= \cf2 StaticText\cf3 (c, \cf2 Rect\cf3 (0, 55, 90, 40))\
		.mouseDownAction_(\{ \cf4 |view, x, y, mod|\cf3 \
			\cf7 //open soundfile\cf3 \
			\cf7 //set title\cf3 \
			if (mod.isShift, \{loadDialog.value\});\
			\cf7 //("mouseDown in CompositeView"++mod).postln;\cf3 \
			if (mod.isAlt, \{\cf8 this\cf3 .free2; \cf5 "remove file"\cf3 .postln; knob1.value= 0; \});\
			if (mod.isCtrl, \{\
				view.enabled= \cf6 true\cf3 ;\
				\cf8 this\cf3 .reloadSample;\
				view.enabled= \cf6 true\cf3 ; \
				...
				\}); \cf7 //reload\cf3 \
		\})\
		.string_(\cf5 "empty"\cf3 )\
		\cf7 //.font_( Font("Monaco", 10))\cf3 \
		.align_( \cf9 \\center\cf3  ).stringColor_(\cf2 Color\cf3 .grey);\
		\
		\cf7 /*\
		...
		*/\cf3 \
		\
		panKnob= \cf2 Knob\cf3 (c, \cf2 Rect\cf3 (15, 100, 15, 15))\
		...
		.receiveDragHandler_(\{\cf4 |view|\cf3 \
			...
			if (drag.class == \cf2 UC33LoopChan\cf3 , \{\
				drag.setAction(\{\cf4 |v|\cf3 \{ panKnob.valueAction_(v.value)\}.defer \}, \cf5 "k"\cf3 +knobNum);\
				\cf7 //drag.action= \{|v| \{ panKnob.valueAction_(v.value)\}.defer \};\cf3 \
			\});\
		\});\
		\
		outbut= \cf2 Button\cf3 (c, \cf2 Rect\cf3 (45, 100, 30, 15))\
...
\};\
	\
		c.onClose= \{\
			if(syn.isNil.not, \{\{\cf8 syn\cf3 .free\}.try; \});\
			\
			if (buffer.isNil.not, \{\
...
setBuf\{\cf4 |buf|\cf3 \
	...
	s.sync;\
	if (dur > 7, \{\
		\cf7 //\{buffer.close; buffer.free\}.try;\cf3 \
...
setValues\{ \cf4 |vals|\cf3 \
	...
	if (vals[0].isNil.not, \{ \
			...
			1.wait;\
			if (vals[4].isNil.not, \{\
			if (vals[2].isNil.not, \{ pan= vals[2]; panKnob.value= \cf9 \\pan\cf3 .asSpec.unmap(vals[2]); \}, \{\
...
progress\{\cf4 |val|\cf3 \
	...
	if (val == 1, \{ if(progressView.isClosed, \{ progressFunc.value(c, \cf2 Rect\cf3 (25, 20, 50, 50));\})\});\
	\}\
\
...
\
setAction\{\cf4 |action, knobNumber|\cf3 \
	knob1.action= action;\
	title.string= \cf5 "PAN"\cf3 +knobNumber;\
	\}\
\
\
\}\

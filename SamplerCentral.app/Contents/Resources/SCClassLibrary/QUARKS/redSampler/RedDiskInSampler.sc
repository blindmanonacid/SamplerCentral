		...
		StartUp.add{
			8.do{|i|								//change here for more channels than 8
				SynthDef("redDiskInSampler-"++(i+1), {
					|i_out= 0, bufnum, amp= 0.7, attack= 0.01, sustain, release= 0.1, gate= 1, pan= 0, speed= 1, dur= 10|
					var src= VDiskIn.ar(i+1, bufnum, speed.abs.lag(0.3) * BufRateScale.kr(bufnum), 0);
					var env= EnvGen.kr(
				...
				SynthDef("redDiskInSampler-"++(i+1)++"loop", {
					|i_out= 0, bufnum, amp= 0.7, attack= 0.01, release= 0.1, gate= 1, pan= 0, speed= 1, loopId, dur= 10|
					var src= VDiskIn.ar(i+1, bufnum, speed.abs.lag(0.3) * BufRateScale.kr(bufnum), 1);
					var env= EnvGen.kr(
					...
					SendTrig.kr(Impulse.kr(1/dur)*EnvGen.kr(Env.new([0, 0, 1], [0.01, 0.01])), loopId); //send trigger when loop starts again
					if (i== 0, {
						Out.ar(i_out, Pan2.ar(src*env*amp, pan));
						},{
						Out.ar(i_out, Balance2.ar(src[0]*env*amp, src[1]*env*amp, pan));
				...
				SynthDef("redDiskInSampler-"++(i+1)++"Trigger", {
					...
					if (i== 0, {
						SendTrig.kr(Coyote.kr(src, minDur: 0.41, thresh: 0.05, fastMul: 0.6), id, 1);
						SendTrig.kr(A2K.kr(DetectSilence.ar(src, 0.01, 0.1)), id, 0);
						},{
						SendTrig.kr(Coyote.kr(src[0], minDur: 0.41, thresh: 0.05, fastMul: 0.6), id, 1);
						SendTrig.kr(A2K.kr(DetectSilence.ar(src[0], 0.01, 0.1)), id, 0);
					});
					if (i== 0, {
						Out.ar(i_out, Pan2.ar(src*env*amp, pan));
						},{
...
RedDiskInSamplerVoice : RedAbstractSamplerVoice {
	...
	play {|attack, sustain, release, amp, out, group, loop, pan, ctrl, speed, id, loopId|

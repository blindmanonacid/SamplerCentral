...
\cf4 var\cf3  samplerFiles, samplerNames, buttonColors, buttonColorsNeg;\
...
\cf6 ~ipDict\cf3 = \cf5 Dictionary\cf3 .newFrom( \cf5 List\cf3 [\
	...
	\cf8 \\headBotMove\cf3 , [\cf9 "172.16.1.180"\cf3 , 8888],\
...
\cf6 ~robotDict\cf3 = ( \cf2 // label, controlSpec, initValues ([on, off], type, duration) // dur up 4 grabs!\cf3 \
	...
	\cf8 \\conductor\cf3 : [\cf8 \\SR3\cf3 , [0, 127, \cf8 \\lin\cf3 , 1].asSpec, [[80, 100], 0, 0.3]],\
...
\
samplerNames= samplerFiles.collect(\{\cf4 |it|\cf3  it.basename.removeExtension\});\
\
\cf6 ~openSamplers\cf3 = \cf5 Dictionary\cf3 .new;\
samplerNames.do(\{\cf4 |it|\cf3 \
...
\
w= \cf5 Window\cf3 (\cf9 ""\cf3 , \cf5 Rect\cf3 (1180, 655, 100, winHeight));\
wb= w.bounds;\
...
buttonColors= samplerNames.collect(\{\cf4 |it|\cf3 \
...
nameList.size.do(\{ \cf4 |i|\cf3 \
	...
	.action_(\{\
		if(\cf6 ~openSamplers\cf3 [samplerNames[i].asSymbol], \{actionList[i].load; \cf6 ~openSamplers\cf3 [samplerNames[i].asSymbol]= \cf7 false\cf3  \})\
	\})\
...
\
\cf5 Button\cf3 (c, 50@15)\
.states_([ [ \cf9 "recAbleton"\cf3 , \cf5 Color\cf3 .black, \cf5 Color\cf3 .yellow ] ])\
.action_(\{\cf4 |v|\cf3 \
	if(\cf6 ~ableton2oscOpen\cf3 == \cf7 false\cf3 , \{\
		((\cf5 String\cf3 .scDir).dirname++\cf9 "/ableton2osc.scd"\cf3 ).standardizePath.load;\
	\});\
\});\
\
\cf5 Button\cf3 (c, 50@15)\
.states_([ [ \cf9 "recJoystick"\cf3 , \cf5 Color\cf3 .black, \cf5 Color\cf3 .yellow ] ])\
.action_(\{\cf4 |v|\cf3 \
	if(\cf6 ~joy2oscOpen\cf3 == \cf7 false\cf3 , \{\
		((\cf5 String\cf3 .scDir).dirname++\cf9 "/joy2osc2.scd"\cf3 ).standardizePath.load;\
	\});\
\});\
\
\cf5 Button\cf3 (c, 50@15)\
		...
		if(v.value==1, \{\
			\cf5 MIDIClient\cf3 .sources.do(\{\cf4 |it, i|\cf3 \
				...
				if(it.name.contains(\cf9 "IAC"\cf3 ), \{\
					...
					\cf2 //[oscDict[busNum], uid, ~ipDict[oscDict[busNum]]].postln;\cf3 \
...
\
\cf5 Button\cf3 (c, 50@15)\
	.states_([[\cf9 "conduct OFF"\cf3 , \cf5 Color\cf3 .white,\cf5 Color\cf3 .grey], [\cf9 "conduct ON"\cf3 , \cf5 Color\cf3 .white,\cf5 Color\cf3 .green]])\
	.action_(\{ \cf4 |v|\cf3 \
		if(v.value==1, \{\

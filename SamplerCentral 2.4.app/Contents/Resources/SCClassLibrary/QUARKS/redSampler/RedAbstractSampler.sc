	...
	*new {|server|								//if server=nil it will use Server.default
	...
	play {|key, attack= 0, sustain, release= 0, amp= 0.7, out= 0, group, loop= 0, pan= 0, speed= 1, offset= 0, startLoop= 0, endLoop= 1, id|
		...
		if(voc.isNil, {
			...
			voc.play(attack, sustain, release, amp, out, group, loop, pan, speed, offset, startLoop, endLoop, id);
		});
	}
	...
	speed_ {|val|
		keys.do{|voices| voices.do{|x| x.speed_(val)}};
	}
	pan_{|val|
		keys.do{|voices| voices.do{|x| x.pan_(val)}};
	}
	flush {|release= 0.4|
...
RedAbstractSamplerVoice {
	...
	*new {|server, path, channels, startFrame, numFrames, length|
	...
	play {|attack= 0, sustain, release= 0, amp= 0.7, out= 0, group, loop= 0, speed= 1, pan= 0, offset= 0, startLoop= 0, endLoop= 1, id, loopId|
		^this.subclassResponsibility(thisMethod)
	}
	amp_ {|val|
		synth.set(\amp, val);
	...
	offset_ {|val|
		synth.set(\offset, val);
	}
	startLoop_ {|val|
		synth.set(\startLoop, val);
	}
	endLoop_ {|val|
		synth.set(\endLoop, val);
	}
	stop {|release= 0.4|
		isReleased= true;
